...
main(int argc, char *argv[])
{
  ...
  int seed_specified=0;
   ...
   while ((i=getopt_long(argc, argv, "-ab:B:d:D:EF:gh::H:K:l:L:M:o:p:r:fsStTvwm:",
	 ...
	 break;
...
#if HASHING
       case 'M' : memory = strtol(optarg, NULL, 0); break;
       ...
       case 'F' : fourlib_depth=atoi(optarg); break;
       case 'K' : fourlib_depth=atoi(optarg); break;
       case 'L' : until = optarg;
	 ...
	 break;
       case 'h' : 
         ...
         {           /*in the default behavior of getopt_long with optional args
           ...
           if(!strncmp(optarg,"debug",5)) fprintf(stderr, USAGE_DEBUG);
         }else
           ...
           if (optind < argc)
           {
               ...
               if(!strncmp(argv[optind],"debug",5)) fprintf(stderr, USAGE_DEBUG);
           }else
	     show_help();
	 return EXIT_SUCCESS;
	 break;
